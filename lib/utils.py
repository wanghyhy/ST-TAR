...
def mask_loss(predicts, classify_predicts, labels, region_mask, bfc, inter_loss, aux_label,  data_type="nyc"):
    """
    Arguments:
    ...
    if data_type == 'nyc':
        #aux_loss = 0.02 * F.cross_entropy(cat_output, aux_label.long())
        inter_loss_sum= 1e-2 * inter_loss
        base_loss= torch.mean(mse_list[0]) + torch.mean(mse_list[1]) + torch.mean(mse_list[2]) + torch.mean(mse_list[3]) + \
            3e-4 * bce_list[0] + 3e-4 * bce_list[1] + 1e-5 * bce_list[2] + 1e-5 * bce_list[3] + mse_fc 
        print('inter_loss_sum',inter_loss_sum.item(),'base_loss',base_loss.item())
        return base_loss + inter_loss_sum # best

    if data_type == 'chicago':
        #aux_loss = 0.02 * F.cross_entropy(cat_output, aux_label.long())
        inter_loss_sum= 1e-2 * inter_loss
        base_loss= torch.mean(mse_list[0]) + 3e-4 * torch.mean(mse_list[1]) + 1e-4 * torch.mean(
            mse_list[2]) + 3e-5 * torch.mean(mse_list[3]) + \
            1e-3 * bce_list[0] + 1e-3 * bce_list[1] + 1e-5 * bce_list[2] + 1e-5 * bce_list[3] + 3e-4 * mse_fc
        print('inter_loss_sum',inter_loss_sum.item(),'base_loss',base_loss.item())
        return base_loss + inter_loss_sum # best
...
def compute_loss(net, dataloader, risk_mask, road_adj, risk_adj, poi_adj,
    ...
    for batch_1, batch_2, batch_3, batch_4 in zip(dataloader[0], dataloader[1], dataloader[2], dataloader[3]):
        ...
        for i in range(4):
            ...
            t_feature, t_target_time, t_graph_feature, t_label, t_aux_label = t_feature.to(device), \
                t_target_time.to(device), t_graph_feature.to(device), t_label.to(device), t_aux_label.to(device)
            if i==0: 
        ...
        final_output, classification_output, inter_loss= net(feature, target_time, graph_feature, road_adj, risk_adj, poi_adj, grid_node_map, trans)
        l = mask_loss(final_output, classification_output, label, risk_mask, bfc, inter_loss, aux_label, data_type)
