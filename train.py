...
parser.add_argument("--config", type=str, help='configuration file',default="config/chicago/Chicago_Config.json")
#parser.add_argument("--config", type=str, help='configuration file',default="config/nyc/NYC_Config.json")
...
def training(net,
    ...
    for epoch in range(1, training_epoch + 1):
        ...
        for batch_1, batch_2, batch_3, batch_4 in zip(train_loader[0], train_loader[1], train_loader[2], train_loader[3]):
            ...
            start_time = time()
            final_output, classification_output, inter_loss = net(r_train_feature, w_train_feature, target_time, road_adj, risk_adj,poi_adj, grid_node_map, trans)
            l = mask_loss(final_output, classification_output, train_label, risk_mask, bfc, inter_loss, aux_label, data_type)
            trainer.zero_grad()
            l.backward()
            trainer.step()
...
def main(config):
    ...
    model = STTAR( transformer_hidden_size, pre_len, 
                   time_shape[0][1], graph_feature_shape[0][2], nums_of_filter, north_south_map, west_east_map, num_of_heads)
